# This Script Pulls Veeam backup Job Session Data from The Veeam Backup Enterprise Manager REST API and parses it into a modern looking static html page

# Veeam Enterprise Manager API Information
$veeamRestServer = "ServerHostname"
$veeamRestPort = "9398" # Default Port
$veeamUsername = "yourusername"
$veeamPassword = "yourpassword"

# CSV Output File - Update the path accordingly
$outputCsv = "MorningReport.csv"

# Path to save the HTML report
$outputHtmlPath = "MorningReportWeb.html"
...
# Authentication URL
$loginUrl = "https://$($veeamRestServer):$($veeamRestPort)/api/sessionMngr/?v=latest"

# API Headers with Authentication
$response = Invoke-WebRequest -Uri $loginUrl -Method Post -Headers @{
...
$hoursBack = 24
$utcNow = (Get-Date).ToUniversalTime()  # Use UTC for accurate filtering
$startDate = $utcNow.AddHours(-$hoursBack)

# Prepare CSV headers
"BackupJob,Server,StartTime,EndTime,Duration,Status,Progress,Info" | Out-File -FilePath $outputCsv -Force

# Get Backup Sessions from Veeam API
$backupSessionsUrl = "https://$($veeamRestServer):$($veeamRestPort)/api/backupSessions?format=Entity"
try {
    $backupSessions = Invoke-RestMethod -Uri $backupSessionsUrl -Headers $authHeaders -Method Get -ContentType "application/json"
} catch {
    Write-Host "Error fetching backup sessions: $_" -ForegroundColor Red
    exit
}

# Loop through the backup sessions and filter based on the last X hours in UTC
foreach ($session in $backupSessions.BackupJobSessions) {
    $startTimeUTC = [datetime]::Parse($session.CreationTimeUTC).ToUniversalTime()

    # Filter based on the last X hours in UTC
    if ($startTimeUTC -ge $startDate) {
        $jobName = $session.JobName
        $endTime = [datetime]::Parse($session.EndTimeUTC).ToUniversalTime()

        # Calculate the duration if start and end times are valid
        $duration = ($endTime - $startTimeUTC).ToString("hh\:mm")

        $status = $session.Result
        $server = $session.Links[0].Name
        $info = $session.Description

        # Adjust status and progress based on the result
        switch ($status) {
            "Success" {
                $status = "Success"
                $progress = "100%"
            ...
            "Warning" {
                $status = "Warning"
                $progress = "100%"
                $info = "Job completed with warnings"
            }
            "Failed" {
                $status = "Failed"
                $progress = "100%"
                $info = $session.ResultMessage
            }
            default {
                $status = "Unknown"
                $progress = "N/A"
        ...
        # Format CSV row and append to the file
        "$jobName,$server,$startTimeUTC,$endTime,$duration,$status,$progress,$info" | Out-File -Append -FilePath $outputCsv
    }
}
...
# Load CSV data
$jobDetails = Import-Csv -Path $outputCsv
