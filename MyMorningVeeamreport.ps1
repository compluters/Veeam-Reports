#Morning Backup report script - this will report session failures
...
function get-lastbackupjobs {
    ...
    # Collect VM backup sessions
    $backupsJobs_VM = Get-VBRBackupSession -ea Stop | where { $_.EndTime -ge (get-date).AddHours(-$lastHours) -or $_.State -like 'Working' }
    $sessions += $backupsJobs_VM | ForEach-Object { $_.GetTaskSessions() }
    
    # Collect agent backup sessions
    $backupsJobs_Agent = Get-VBRComputerBackupJobSession -ea Stop | where { $_.EndTime -ge (get-date).AddHours(-$lastHours) -or $_.State -like 'Working' }
    $sessions += $backupsJobs_Agent | Get-VBRTaskSession | where { $_.Name -notlike "*-CL*" }

   # Collect backup copy job sessions (replacing tape jobs)
    $backupJobs_Copy = Get-VBRBackupCopyJob | %{ Get-VBRSession -Job $_ } | where { $_.EndTime -ge (get-date).AddHours(-$lastHours) -or $_.State -like 'Working' }
    $sessions += $backupJobs_Copy | Get-VBRTaskSession
    ...
    foreach ($session in $sessions) {
        ...
        $progress = if ($session.Status.ToString() -eq "InProgress") {
        ...
        $info = ''
        if ($session.Status.ToString() -eq "Failed" -or $session.Status.ToString() -eq "Warning") {
...
$outputCsvPath = "backup_job_report.csv"

# Export job details directly to CSV (no console logging)
$jobDetails | Export-Csv -Path $outputCsvPath -NoTypeInformation -Force

Write-Host "Backup job data has been successfully logged to: $outputCsvPath"

### html parse from csv
##########
# Path to the CSV file
$csvFilePath = "backup_job_report.csv"

# Path to save the HTML report
$outputHtmlPath = "backup_report_with_logo.html"
...
Write-Host "Backup report with SVG logo has been generated: $outputHtmlPath"
